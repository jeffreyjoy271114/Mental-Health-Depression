...
# Define the model repository and model file
model_repo = "jeffrey-joy/mental_health_Data"
model_filename = "model.pkl"

# Download the model from Hugging Face
try:
    model_path = hf_hub_download(repo_id=model_repo, filename=model_filename)
    st.success(f"Model downloaded successfully from Hugging Face: {model_path}")
    ...
    st.error(f"An error occurred while downloading/loading the model: {e}")
...
with st.sidebar:
    ...
    Profession = st.selectbox('Profession', ('Student', 'Teacher', 'Professional Workers', 'Content Writer', 'Architect', 'Consultant', 'HR Manager', 'Pharmacist', 'Doctor', 'Business Analyst', 'Chemist', 'Entrepreneur', 'Chef', 'Educational Consultant', 'Data Scientist', 'Researcher', 'Lawyer', 'Customer Support', 'Marketing Manager', 'Pilot', 'Travel Consultant', 'Plumber', 'Sales Executive', 'Manager', 'Judge', 'Electrician', 'Financial Analyst', 'Software Engineer', 'Civil Engineer', 'UX/UI Designer', 'Digital Marketer', 'Accountant', 'Financial Analyst', 'Mechanical Engineer', 'Graphic Designer', 'Research Analyst', 'Investment Banker'))
